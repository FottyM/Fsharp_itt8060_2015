...
let rec segments (list:int list) =
        match list with
        |head::tail when head < tail.Head -> (breaker list) :: segments tail
...
[<Measure>] type C
let asimpleList: float<C> list = [100.0<C>]

let rec theTerminator asimpleList (num:float<C>) =
    match asimpleList with 
    |head::tail -> if num > head then theTerminator tail head
                    else head::num::tail

theTerminator asimpleList 101.0<C>
